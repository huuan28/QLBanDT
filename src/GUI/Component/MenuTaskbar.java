...
public class MenuTaskbar extends JPanel {
    ...
    ThongKe thongKe;
    String[][] getSt = {
        {"Trang chủ", "home.svg", "trangchu"},//0
        {"Sản phẩm", "product.svg", "sanpham"},//1
        {"Thuộc tính", "brand.svg", "thuoctinh"},//2
        {"Khu vực kho", "area.svg", "khuvuckho"},//3
        {"Nhập hàng", "import.svg", "nhaphang"},//4
        {"Hóa đơn", "export.svg", "xuathang"},//5
        {"Khách hàng", "customer.svg", "khachhang"},//6
        {"Nhà cung cấp", "supplier.svg", "nhacungcap"},//
        {"Nhân viên", "staff.svg", "nhanvien"},//8
        {"Tài khoản", "account.svg", "taikhoan"},//9
        {"Thống kê", "statistical.svg", "thongke"},//10
        {"Phân quyền", "permission.svg", "nhomquyen"},//11
        {"Đăng xuất", "log_out.svg", "dangxuat"},//12
    };
...
//        pnlCenter.setBorder(new EmptyBorder(0,15,0,35));
        ...
        //listitem[10].setVisible(false); //thong ke
        listitem[11].setVisible(false); //phan quyen
        //
        listitem[0].setBackground(HowerBackgroundColor);
        listitem[0].setForeground(HowerFontColor);
        listitem[0].isSelected = true;

        //tạo hiệu ứng hover cho từng thành phần
        for (int i = 0; i < getSt.length; i++) {
    ...
    //kiểm tra xem user có quyền sử dụng hay không
