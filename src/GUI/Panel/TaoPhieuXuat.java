...
public final class TaoPhieuXuat extends JPanel {
    ...
    private JTextField txtTichluy;
    private JTextField tfquydoi;
    ...
    private void initComponent(String type) {
        ...
        manv = tk.getManv();
        txtMaphieu.setText("HD" + PhieuXuatDAO.getInstance().getAutoIncrement());
        ...
        JPanel khPanel = new JPanel(new GridLayout(7, 1, 5, 0));
        khPanel.setBackground(Color.WHITE);
        khPanel.setPreferredSize(new Dimension(0, 300));
        ...
        JLabel lbltichluy = new JLabel("Điểm tích lũy");
        JLabel lblgiam = new JLabel("Giảm: ");
        JLabel lblquydoi = new JLabel("Đổi điểm");
        txtTichluy = new JTextField("");
        txtTichluy.setEditable(false);
        tfquydoi = new JTextField("");

        lbltichluy.setBorder(new EmptyBorder(0, 10, 0, 10));

        lblquydoi.setBorder(new EmptyBorder(0, 10, 0, 10));
        lblgiam.setBorder(new EmptyBorder(0, 10, 0, 10));

        khPanel.add(lbltichluy);

        JPanel txtPanel = new JPanel(new BorderLayout());
        txtPanel.add(txtTichluy, BorderLayout.CENTER);
        txtPanel.setBackground(Color.WHITE);
        txtPanel.setBorder(new EmptyBorder(0, 10, 0, 10));  // Add padding to align with lbltichluy
        khPanel.add(txtPanel);
        khPanel.add(lblquydoi);

        JPanel pnquydoi = new JPanel(new BorderLayout());
        pnquydoi.setBorder(new EmptyBorder(0, 10, 0, 10));
        pnquydoi.setPreferredSize(new Dimension(0, 40));
        pnquydoi.setOpaque(false);

        JPanel pnNutquydoi = new JPanel(new GridLayout(1, 1));
        pnNutquydoi.setOpaque(false);
        pnNutquydoi.setPreferredSize(new Dimension(60, 0));
        tfquydoi.setEditable(false);
        ButtonCustom btnDoi = new ButtonCustom("Đổi", "success", 14);
        btnDoi.addActionListener((ActionEvent e) -> {
            int temp1 = sum;
            doi = Integer.parseInt(tfquydoi.getText());
            lblgiam.setText("Giảm: " + Formater.FormatVND(doi));
            sum -= doi;
            if (sum < 0) {
                sum = temp1;
                lblgiam.setText("Giảm: " + Formater.FormatVND(0));
            }

            lbltongtien.setText(Formater.FormatVND(sum));
        });
        pnNutquydoi.add(btnDoi);

        pnquydoi.add(pnNutquydoi, BorderLayout.EAST);
        pnquydoi.add(tfquydoi, BorderLayout.CENTER);

        khPanel.add(pnquydoi);
        khPanel.add(lblgiam);
        ...
        btnNhapHang.addActionListener((ActionEvent e) -> {
            if (chitietphieu.isEmpty()) {
                ...
                if (input == 0) {
                    ...
                    tichluy-=doi;
                    khachhang.setTichluy(tichluy + sum / 10000);
    ...
    public void setKhachHang(int index) {
        makh = index;
        khachhang = khachHangBUS.selectKh(makh);
        tichluy = khachhang.getTichluy();
        txtKh.setText(khachhang.getHoten());
        txtTichluy.setText(tichluy + "");
        tfquydoi.setEditable(true);
        ((AbstractDocument) tfquydoi.getDocument()).setDocumentFilter(new Fillter(tfquydoi, tichluy));
    }
