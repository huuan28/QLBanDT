...
public class ThongKeDAO {
    ...
    public ArrayList<ThongKeNhapDTO> getThongKeNhap(String start, String end) {
        ArrayList<ThongKeNhapDTO> result = new ArrayList<>();
    try {
        Connection con = JDBCUtil.getConnection();

        String sql = "SELECT DATE(thoigian) AS ngay, SUM(tongtien) AS chiphi " +
                     "FROM phieunhap " +
        ...
        pstSelect.setString(2, end);

        ResultSet rs = pstSelect.executeQuery();
        while (rs.next()) {
            Date ngay = rs.getDate("ngay");
            int chiphi = rs.getInt("chiphi");
            ThongKeNhapDTO tn = new ThongKeNhapDTO(ngay, chiphi);
            result.add(tn);
        }
    } catch (SQLException e) {
        e.printStackTrace();
