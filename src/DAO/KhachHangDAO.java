...
public class KhachHangDAO implements DAOinterface<KhachHangDTO> {
    ...
    public int update(KhachHangDTO t) {
        ...
        try {
            Connection con = (Connection) JDBCUtil.getConnection();
            String sql = "UPDATE `khachhang` SET `makh`=?,`tenkhachhang`=?,`diachi`=?,`sdt`=?, `tichluy`=? WHERE makh=?";
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
            pst.setInt(1, t.getMaKH());
            pst.setString(2, t.getHoten());
            pst.setString(3, t.getDiachi());
            pst.setString(4, t.getSdt());
            pst.setInt(5, t.getTichluy());
    ...
    public ArrayList<KhachHangDTO> selectAll() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                int tichluy = rs.getInt("tichluy");
                KhachHangDTO kh = new KhachHangDTO(makh, tenkhachhang, sdt, diachi, ngaythamgia, tichluy);
                result.add(kh);
            }
            JDBCUtil.closeConnection(con);
        } catch (Exception e) {
    ...
    public KhachHangDTO selectById(String t) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Date ngaythamgia = rs.getDate("ngaythamgia");
                int tichluy = rs.getInt("tichluy");
                result = new KhachHangDTO(makh, tenkhachhang, sdt, diachi, ngaythamgia, tichluy);
            }
            JDBCUtil.closeConnection(con);
        } catch (Exception e) {
            System.out.println(e);
    ...
    public int getAutoIncrement() {
        ...
        try {
            Connection con = (Connection) JDBCUtil.getConnection();
            String sql = "SELECT `AUTO_INCREMENT` FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'qlbandt' AND   TABLE_NAME   = 'khachhang'";
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
